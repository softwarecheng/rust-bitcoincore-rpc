...
use bitcoint4::absolute::LockTime;
use bitcoint4::address::NetworkChecked;

use crate::json::BlockStatsFields as BsFields;
use bitcoincore_rpc::bitcoincore_rpc_json::{
    GetBlockTemplateModes, GetBlockTemplateRules, ScanTxOutRequest,
};
use bitcoint4::consensus::encode::{deserialize, serialize_hex};
use bitcoint4::hashes::hex::FromHex;
use bitcoint4::hashes::Hash;
use bitcoint4::{secp256k1, sighash, ScriptBuf};
use bitcoint4::{
...
fn test_get_new_address(cl: &Client) {
    ...
    assert_eq!(addr.address_type(), Some(bitcoint4::AddressType::P2pkh));

    let addr = cl.get_new_address(None, Some(json::AddressType::Bech32)).unwrap().assume_checked();
    assert_eq!(addr.address_type(), Some(bitcoint4::AddressType::P2wpkh));

    let addr =
        cl.get_new_address(None, Some(json::AddressType::P2shSegwit)).unwrap().assume_checked();
    assert_eq!(addr.address_type(), Some(bitcoint4::AddressType::P2sh));
}
