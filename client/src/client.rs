...
use crate::{bitcoint4, deserialize_hex};
use bitcoint4_private::hex::exts::DisplayHex;
use jsonrpc;
use serde;
use serde_json;

use crate::bitcoint4::address::{NetworkChecked, NetworkUnchecked};
use crate::bitcoint4::hashes::hex::FromHex;
use crate::bitcoint4::secp256k1::ecdsa::Signature;
use crate::bitcoint4::{
...
impl<'a> RawTx for &'a Transaction {
    fn raw_hex(self) -> String {
        bitcoint4::consensus::encode::serialize_hex(self)
    }
}
...
pub trait RpcApi: Sized {
    ...
    fn get_block(&self, hash: &bitcoint4::BlockHash) -> Result<Block> {
        let hex: String = self.call("getblock", &[into_json(hash)?, 0.into()])?;
        deserialize_hex(&hex)
    }

    fn get_block_hex(&self, hash: &bitcoint4::BlockHash) -> Result<String> {
        self.call("getblock", &[into_json(hash)?, 0.into()])
    }

    fn get_block_info(&self, hash: &bitcoint4::BlockHash) -> Result<json::GetBlockResult> {
        self.call("getblock", &[into_json(hash)?, 1.into()])
    }
    //TODO(stevenroose) add getblock_txs

    fn get_block_header(&self, hash: &bitcoint4::BlockHash) -> Result<bitcoint4::block::Header> {
    ...
    fn get_best_block_hash(&self) -> Result<bitcoint4::BlockHash> {
        self.call("getbestblockhash", &[])
    }

    /// Get block hash at a given height
    fn get_block_hash(&self, height: u64) -> Result<bitcoint4::BlockHash> {
    ...
    fn get_raw_transaction(
        &self,
        txid: &bitcoint4::Txid,
        block_hash: Option<&bitcoint4::BlockHash>,
    ) -> Result<Transaction> {
    ...
    fn get_block_filter(
        &self,
        block_hash: &bitcoint4::BlockHash,
    ...
    fn list_since_block(
        &self,
        blockhash: Option<&bitcoint4::BlockHash>,
    ...
    ) -> Result<Vec<bitcoint4::BlockHash>> {
    ...
    fn generate(&self, block_num: u64, maxtries: Option<u64>) -> Result<Vec<bitcoint4::BlockHash>> {
        self.call("generate", &[block_num.into(), opt_into_json(maxtries)?])
    }

    /// Mark a block as invalid by `block_hash`
    fn invalidate_block(&self, block_hash: &bitcoint4::BlockHash) -> Result<()> {
        self.call("invalidateblock", &[into_json(block_hash)?])
    }

    /// Mark a block as valid by `block_hash`
    fn reconsider_block(&self, block_hash: &bitcoint4::BlockHash) -> Result<()> {
    ...
    fn get_raw_mempool(&self) -> Result<Vec<bitcoint4::Txid>> {
    ...
    fn get_raw_mempool_verbose(
        &self,
    ) -> Result<HashMap<bitcoint4::Txid, json::GetMempoolEntryResult>> {
        self.call("getrawmempool", &[into_json(true)?])
    }

    /// Get mempool data for given transaction
    fn get_mempool_entry(&self, txid: &bitcoint4::Txid) -> Result<json::GetMempoolEntryResult> {
    ...
    fn send_raw_transaction<R: RawTx>(&self, tx: R) -> Result<bitcoint4::Txid> {
    ...
    fn wallet_process_psbt(
        ...
        let defaults = [
            true.into(),
            into_json(json::SigHashType::from(bitcoint4::sighash::EcdsaSighashType::All))?,
            true.into(),
        ];
    ...
    /// Submit a block
    fn submit_block(&self, block: &bitcoint4::Block) -> Result<()> {
        let block_hex: String = bitcoint4::consensus::encode::serialize_hex(block);
...
mod tests {
    ...
    #[test]
    fn test_raw_tx() {
        use crate::bitcoint4::consensus::encode;
        let client = Client::new("http://localhost/".into(), Auth::None).unwrap();
        let tx: bitcoint4::Transaction = encode::deserialize(&Vec::<u8>::from_hex("").unwrap()).unwrap();
